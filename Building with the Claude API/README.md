...
Jupyter notebooks for learning the [Anthropic Claude API](https://docs.anthropic.com/). Each notebook walks through a single concept with runnable examples.

For repo-wide setup (Python venv, `ANTHROPIC_API_KEY`, sanity check, troubleshooting), see the [root README](../README.md).
...
> This project uses [`uv`](https://docs.astral.sh/uv/) for dependency management (not `pip`). See [`app_starter/README.md`](app_starter/README.md) for the exact commands.
...
> 🔑 **The RAG notebooks require a `VOYAGE_API_KEY`** in addition to `ANTHROPIC_API_KEY`. Get one at <https://dashboard.voyageai.com/>, then add it to your `.env` (see [`VoyageAI_API_Key_Directions.pdf`](Intro%20Retrival%20Augmented%20Generation/VoyageAI_API_Key_Directions.pdf) for step-by-step instructions):
>
> ```bash
...
| File | Topic |
| --- | --- |
| [`Quiz_onAgents_Workflows.md`](Quiz_onAgents_Workflows.md) | Quiz on agent vs. workflow patterns (chaining, routing, parallelization, evaluator-optimizer) |

### Final Assessment

| File | Topic |
| --- | --- |
| [`Final_Assessment.md`](Final_Assessment.md) | End-of-path quiz covering tool use, prompt engineering, evaluation, MCP, and agents/workflows |
